...
class OpenAIGitHelper:
    ...
    def __init__(self):
        """初期化処理"""
        # 設定の読み込み
        self.use_openai = self._get_config_value("GIT", "use_openai", "true").lower() == "true"
    ...
    def _generate_branch_strategy_hint(self, repo_path: str, current_branch: str) -> str:
        ...
        try:
            ...
            # リポジトリの構造情報
            file_count_result = self._run_git_command(['git', 'ls-files', '|', 'wc', '-l'], cwd=repo_path, shell=True)
